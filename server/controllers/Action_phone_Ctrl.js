const Action_phone = require("../models/Action_phone");
const sendEmail = require("../utils/sendEmail");
exports.createOne = async (req, res, next) => {
  const result = new Action_phone({
    data: {
      name: req.body.name,
    ...
    html: `
     ...
     <body>
          ...
          <h2><b>Bestellen Sie hier einen Anruf und wir werden Sie zurückrufen:</b></h2>
            ...
            <h2>Phone: <a href="tel:${result.data.phone}">${result.data.phone}</a></h2>
         </div>
     </body>`,
  };
  try {
...
exports.updateOne = async (req, res, next) => {
  const result = await Action_phone.findByIdAndUpdate(req.params.id);
...
exports.deleteOne = async (req, res, next) => {
  await Action_phone.findByIdAndDelete({ _id: req.params.id });
  res.status(200).json([]);
};

exports.getItem = async (req, res, next) => {
  const result = await Action_phone.findById(req.params.id);
  res.status(200).json(result);
};
exports.getItems = async (req, res, next) => {
  const result = await Action_phone.find();
  res.status(200).json(result);
};
