const Images_hotel = require('../models/Images_hotel');
...
exports.deleteOne=async (req,res)=>{
    await Images_hotel.findById(req.params.id)
        .exec((error,data)=>{
            if(error){res.send(error)}
                ...
                fs.unlink(filePath, async (err)=>{
                    if(err) throw err
                    await Images_hotel.findByIdAndDelete(req.params.id)
                    res.status(200).json({
                        success:true,
                        data:"Success delete"
                    })
